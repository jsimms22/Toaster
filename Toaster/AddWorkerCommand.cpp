...
void AddWorkerCommand::ExecuteCommand(CommandContext& ctx, const dpp::slashcommand_t& event)
{
    ...
    {
        event.reply(dpp::message(fmt::format("User <@{}> is already assigned to this job request.\n", worker)).set_flags(dpp::m_ephemeral));
        return;
    }
